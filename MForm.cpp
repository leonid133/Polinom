...
{
   n = Form1->LabeledEdit1->Text.ToIntDef(0);
   flEdit = true;
}
...
void __fastcall TForm1::FormResize(TObject *Sender)
{
   ...
   {
      SG_data->Width = (SG_data->DefaultColWidth+2)*(LabeledEdit2->Text.ToInt()+1);
      SG_data->Height = 3*(SG_data->DefaultRowHeight+2);
   }
   else
   {
      SG_data->Width = (ClientWidth - 20);
      SG_data->Height = 3*(SG_data->DefaultRowHeight+2)+22;
   }
   ...
   {
      SG_koef->Width = (SG_koef->DefaultColWidth+2)*(Form1->LabeledEdit1->Text.ToIntDef(0) + 1) + 3;
      SG_koef->Height = 2*(SG_koef->DefaultRowHeight+2);
   }
   else
   {
      SG_koef->Width = (ClientWidth - 20);
      SG_koef->Height = 2*(SG_koef->DefaultRowHeight+2) + 22;
   }
...
//--------------------------------------------------------------------------
void TForm1::ExpExel(void)
{
      Variant  vVarApp,vVarBooks,vVarBook,
               vVarSheets,vVarSheet,vVarCell, vVarCell2, Range;
      ...
      catch (...)
      {
        ...
        try
        {
                ...
                vVarSheet=vVarApp.OlePropertyGet("WorkSheets",1);
        } catch(...)
        {
      ...
      vVarCell=vVarSheet.OlePropertyGet("Cells").
               ...
               vVarCell.OlePropertySet("Value", "Х");
      vVarCell=vVarSheet.OlePropertyGet("Cells").
                         OlePropertyGet("Item",1,3);
               vVarCell.OlePropertySet("Value", "У");
      vVarCell=vVarSheet.OlePropertyGet("Cells").
                         OlePropertyGet("Item",1,5);
               vVarCell.OlePropertySet("Value", "Степень");
      vVarCell=vVarSheet.OlePropertyGet("Cells").
                         OlePropertyGet("Item",1,6);
               vVarCell.OlePropertySet("Value", "Коэф");

      for (int yi=0; yi<nData; yi++)
      {
         vVarCell=vVarSheet.OlePropertyGet("Cells").
                            OlePropertyGet("Item",yi+2,1);
                  vVarCell.OlePropertySet("Value", yi);
         vVarCell=vVarSheet.OlePropertyGet("Cells").
                            OlePropertyGet("Item",yi+2,2);
                  vVarCell.OlePropertySet("Value", x[yi]);
         vVarCell=vVarSheet.OlePropertyGet("Cells").
                            OlePropertyGet("Item",yi+2,3);
                  vVarCell.OlePropertySet("Value", y[yi]);
      }
      for (int yi=0; yi<n+1; yi++)
      {
         vVarCell=vVarSheet.OlePropertyGet("Cells").
                            OlePropertyGet("Item",yi+2,5);
                  vVarCell.OlePropertySet("Value", yi);
         vVarCell=vVarSheet.OlePropertyGet("Cells").
                            OlePropertyGet("Item",yi+2,6);
                  vVarCell.OlePropertySet("Value", a[yi]);
      }
}
//--------------------------------------------------------------------------
